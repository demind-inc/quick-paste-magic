...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!workspaceId || !email?.trim()) {
      return Response.json({ error: "workspaceId and email are required" }, { status: 400, headers: corsHeaders });
    }
    const normalizedEmail = email.trim().toLowerCase();
    const inviteRole = role === "viewer" ? "viewer" : "editor";

    const { data: memberRole, error: roleError } = await supabaseAdmin.rpc("get_workspace_role", {
      _user_id: invitedBy,
      _workspace_id: workspaceId,
    });
    if (roleError || memberRole !== "owner") {
      return Response.json({ error: "Only workspace owners can invite members" }, { status: 403, headers: corsHeaders });
    }
    ...
    if (insertError) {
      if (insertError.code === "23505") {
        return Response.json({ error: "An invitation for this email already exists" }, { status: 409, headers: corsHeaders });
      }
      return Response.json({ error: insertError.message }, { status: 400, headers: corsHeaders });
    }

    const redirectTo = `${siteUrl.replace(/\/$/, "")}/accept-invite?token=${invitation.token}`;
