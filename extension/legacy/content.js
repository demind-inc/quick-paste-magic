 ...
 *
 * Shortcut trigger: type `/` followed by snippet shortcut characters.
 * The overlay appears with matching snippets; Enter inserts, Esc dismisses.
 */
...
function renderOverlay(query) {
  ...
  const items = overlay.querySelectorAll(".snipdm-item");
...
function openFillModal(snippet, onInsert) {
...
function insertText(text) {
  ...
  // Remove the typed shortcut buffer from the field first
  if (typedBuffer && activeField.tagName !== "DIV") {
    const start = activeField.selectionStart - typedBuffer.length;
    const end = activeField.selectionStart;
    const current = activeField.value;
    activeField.value = current.slice(0, start) + current.slice(end);
    activeField.selectionStart = activeField.selectionEnd = start;
  }

  if (activeField.tagName === "DIV" && activeField.isContentEditable) {
    ...
    if (nativeSetter) {
      ...
      const current = activeField.value;
      nativeSetter.call(activeField, current.slice(0, start) + text + current.slice(start));
      activeField.selectionStart = activeField.selectionEnd = start + text.length;
...
function selectSnippet(snippet) {
  ...
  openFillModal(snippet, (resolved) => {
    insertText(resolved);
    // Notify background to increment use_count
...
document.addEventListener(
  ...
  (e) => {
    ...
    const isEditable =
      ...
      (field.tagName === "DIV" && field.isContentEditable);
    ...
    const caret = field.tagName === "DIV"
      ? value.length
      : field.selectionStart ?? 0;

    // Find trigger char before caret
    ...
    // Only activate if no space in query (fresh shortcut)
    if (query.includes(" ")) {
      closeOverlay();
      return;
    }
